...
$lang = array_merge($lang, array(
	...
	'ALLOW_BOOKMARKS_EXPLAIN'	=> '會員可以存儲個人收藏夾',
	...
	'ALLOW_SIG_SMILIES'			=> '在會員簽名檔中使用表情圖標',
	'ALLOW_SMILIES'				=> '允許表情圖標',
...
// Avatar Settings
$lang = array_merge($lang, array(
	'ACP_AVATAR_SETTINGS_EXPLAIN'	=> '圖示是會員用於展示自己的一幅小圖片. 根據界面有所不同, 不過它們通常顯示在會員發佈的文章旁邊. 這裡您可以決定會員是否能和如何定義他們的圖示. 請注意如果允許會員上傳圖示, 您必須建立下面的目錄並保證這個目錄對於web伺服器是可寫的. 同時也要注意這個檔案大小只對圖示上傳有效, 對於連結的圖示是沒有限制的.',
...
$lang = array_merge($lang, array(
	...
	'ALLOW_SMILIES_PM'			=> '允許在私人簡訊中使用表情圖標',
	'BOXES_LIMIT'				=> '每個資料夾中的最大信件數量',
	'BOXES_LIMIT_EXPLAIN'		=> '會員每個資料夾中的簡訊將不能超過這個數量. 設定為0將不作限制.',
	'BOXES_MAX'					=> '每個會員的最大資料夾數量',
	'BOXES_MAX_EXPLAIN'			=> '預設情況下會員可以建立資料夾來存儲自己的信件.',
	'ENABLE_PM_ICONS'			=> '在私人簡訊中允許使用主題圖標',
...
// Post Settings
$lang = array_merge($lang, array(
	'ACP_POST_SETTINGS_EXPLAIN'			=> '這裡您可以設定發帖的所有預設設定',
...
//	'ACC_USER_ADMIN'			=> '會員 + 管理員',
	'ALLOW_EMAIL_REUSE'			=> '允許重複使用email地址',
	'ALLOW_EMAIL_REUSE_EXPLAIN'	=> '不同的會員可以使用相同的email註冊.',
	'COPPA'						=> 'COPPA',
	'COPPA_FAX'					=> 'COPPA傳真號',
	'COPPA_MAIL'				=> 'COPPA郵件地址',
	'COPPA_MAIL_EXPLAIN'		=> '這是父母用於發送COPPA註冊資料表的郵件地址',
...
$lang = array_merge($lang, array(
	...
	'VISUAL_CONFIRM_POST'					=> '啟用遊客發帖驗證',
	'VISUAL_CONFIRM_POST_EXPLAIN'			=> '當匿名會員發帖時需要輸入一個隨機產生的驗證碼.',
...
$lang = array_merge($lang, array(
	...
	'LOAD_CPF_MEMBERLIST'			=> '允許界面在會員列表中顯示自定義資料',
	'LOAD_CPF_VIEWPROFILE'			=> '在會員資料中顯示自定義資料',
	'LOAD_CPF_VIEWTOPIC'			=> '在文章檢視中顯示自定義會員資料',
	...
	'YES_ANON_READ_MARKING'			=> '允許遊客標記主題',
	'YES_ANON_READ_MARKING_EXPLAIN'	=> '為遊客存儲已閱讀/未讀狀態. 如果停用, 對於遊客所有文章將顯示為已閱讀.',
	'YES_BIRTHDAYS'					=> '啟用生日列表',
	'YES_JUMPBOX'					=> '顯示跳轉列表',
	'YES_MODERATORS'				=> '顯示討論區版主',
	'YES_ONLINE'					=> '顯示在線上會員',
	'YES_ONLINE_EXPLAIN'			=> '在首頁, 版面和文章中顯示在線上會員資訊.',
	'YES_ONLINE_GUESTS'				=> '檢視在線上會員時顯示遊客在線上資訊',
	'YES_ONLINE_GUESTS_EXPLAIN'		=> '在檢視在線上會員時, 顯示遊客在線上資訊.',
...
// Auth settings
$lang = array_merge($lang, array(
	'ACP_AUTH_SETTINGS_EXPLAIN'	=> 'phpBB 支援認證外掛和模組. 這允許您決定如何驗證會員是否登入. 預設的三個外掛時DB, LDAP和Apache. 並不是所有方式都需要額外資訊, 所以您只需要填寫選中的方式需要的資訊即可.',
	...
	'LDAP_EMAIL_EXPLAIN'			=> '將這個設定為會員的email屬性名稱(如果存在的話), 以便於為新會員自動設定email地址, 保留空白的話將使第一次登入的會員email地址為空.',
	...
	'LDAP_PASSWORD_EXPLAIN'			=> '保留空白以便於匿名瀏覽. 否則為以上會員填入密碼. <strong>警告:</strong> 這個密碼將用明文存儲在資料庫中, 對於可瀏覽資料庫的會員都是可用的.',
	'LDAP_SERVER'					=> 'LDAP 伺服器名稱',
	'LDAP_SERVER_EXPLAIN'			=> '如果使用LDAP, 這是伺服器的主機名或IP地址.',
...
$lang = array_merge($lang, array(
	...
	'CHECK_DNSBL_EXPLAIN'			=> '如果啟用, 會員的IP地址將被檢查以防禦以下在註冊和發帖時的 DNSBL 服務: <a href="http://spamcop.net">spamcop.net</a>, <a href="http://dsbl.org">dsbl.org</a> and <a href="http://spamhaus.org">spamhaus.org</a>. 這個檢查將耗費一些時間, 取決於伺服器的設定. 如果讓討論區變得很漫或產生很多錯誤報告, 請停用這個功能.',
	'CLASS_B'						=> 'A.B',
	'CLASS_C'						=> 'A.B.C',
	...
	'IP_VALID'						=> '對話IP驗證',
	'IP_VALID_EXPLAIN'				=> '決定會員的IP如何用於會話驗證; <samp>所有</samp> 表示完整地址, <samp>A.B.C</samp> 表示開頭的 x.x.x, <samp>A.B</samp> 表示開頭的 x.x, <samp>None</samp> 取消驗證.',
...
// Email Settings
$lang = array_merge($lang, array(
	'ACP_EMAIL_SETTINGS_EXPLAIN'	=> '這個資訊將用於討論區向會員發送email. 請確保email地址是正確的, 任何被退回和無法投遞的消息都將被發送至這個地址. 如果您的主機不提供本地(基於PHP的) email服務, 您可以使用SMTP發送消息. 這需要伺服器的地址 (必要的話詢問提供者). 如果伺服器需要驗證 (並且只有在需要時) 輸入必要的帳號和密碼. 請注意只提供基本的驗證, 暫時不提供其他的驗證支援.',

	'ADMIN_EMAIL'					=> '返回email地址',
	'ADMIN_EMAIL_EXPLAIN'			=> '這將是所有email的返回地址, 技術聯繫email. 將顯示於<samp>Return-Path</samp> 和 <samp>Sender</samp>.',
	'BOARD_EMAIL_FORM'				=> '會員通過討論區發送email',
	'BOARD_EMAIL_FORM_EXPLAIN'		=> '可以使用討論區發送email而不顯示會員的email地址.',
	'BOARD_HIDE_EMAILS'				=> '隱藏email地址',
	'BOARD_HIDE_EMAILS_EXPLAIN'		=> '這個功能使email地址完全隱蔽.',
	'CONTACT_EMAIL'					=> 'email聯繫地址',
