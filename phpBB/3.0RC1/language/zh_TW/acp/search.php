...
$lang = array_merge($lang, array(
	'ACP_SEARCH_INDEX_EXPLAIN'				=> '這裡您可以管理內容索引. 因為一般只使用一個後台, 您可以刪除所有不用的索引. 在改變搜尋設定(例如最小/最大字元串長度)後, 建議重新產生索引以體現修改.',
	'ACP_SEARCH_SETTINGS_EXPLAIN'			=> '這裡您可以設定如何使用搜尋和檢索文章. 您可以設定選項限制搜尋動作的處理器負載. 一部分設定和和搜尋引擎的設定是一樣的.',

	'COMMON_WORD_THRESHOLD'					=> '詞頻閾值',
	'COMMON_WORD_THRESHOLD_EXPLAIN'			=> '在所有的文章中都頻繁出現的單詞將被識別為高頻詞. 高頻詞在搜尋中將被忽略. 設定0則取消這項功能. 只有當文章數大於100時這個設定才能生效.',
	'CONFIRM_SEARCH_BACKEND'				=> '您確認更換搜尋後端嗎? 更換後您需要重新建立索引. 如果您不打算再切換回舊的後端, 您可以刪除原先的索引以釋放空間.',
	'CONTINUE_DELETING_INDEX'				=> '繼續前次的索引刪除',
	'CONTINUE_DELETING_INDEX_EXPLAIN'		=> '存在一個已經開始的索引刪除. 要瀏覽索引頁面您必須先完成它.',
	'CONTINUE_INDEXING'						=> '繼續前次的索引產生',
	'CONTINUE_INDEXING_EXPLAIN'				=> '存在一個已經開始的索引產生. 要瀏覽索引頁面您必須先完成它.',
	'CREATE_INDEX'							=> '建立索引',
	...
	'FULLTEXT_MYSQL_NOT_MYISAM'				=> 'MySQL全文檢索只能在MyISAM格式資料表中使用.',
	...
	'LIMIT_SEARCH_LOAD_EXPLAIN'				=> '如果一分鐘內系統負載超過這個值, 搜尋頁面將無法使用, 1.0 約等於100% 的處理器負載. 這個功能只在基於UNIX/Linux系統的伺服器上有效.',

	'MAX_SEARCH_CHARS'						=> '索引的最大字元長度',
	'MAX_SEARCH_CHARS_EXPLAIN'				=> '大於這個長度的短語將不會被檢索.',
	...
	'SEARCH_STORE_RESULTS_EXPLAIN'			=> '快取的結果在這個時間(秒)後將失效. 設定為0則停用快取.',
	'SEARCH_TYPE'							=> '搜尋後端',
	...
	'YES_SEARCH_UPDATE_EXPLAIN'				=> '當文章發表時更新全文索引, 如果停用檢索這個功能將停用.',
));
