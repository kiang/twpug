...
$lang = array_merge($lang, array(
	'ACP_ATTACHMENT_SETTINGS_EXPLAIN'	=> '這裡您可以設定附加檔案和相關的附加檔案分類.',
	'ACP_EXTENSION_GROUPS_EXPLAIN'		=> '這裡您可以新增, 刪除, 修改或停用副檔名組. 選項包括設定附加檔案類別, 更改下載機制和定義附加檔案組的簡明圖示.',
	'ACP_MANAGE_EXTENSIONS_EXPLAIN'		=> '這裡您可以管理您允許的附加檔案副檔名. 要啟用某個副檔名, 請到副檔名類別管理面板. 請注意不要允許指令檔案 (例如 <code>php</code>, <code>php3</code>, <code>php4</code>, <code>phtml</code>, <code>pl</code>, <code>cgi</code>, <code>py</code>, <code>rb</code>, <code>asp</code>, <code>aspx</code>等類型…)',
	'ACP_ORPHAN_ATTACHMENTS_EXPLAIN'	=> '這裡您可以看見附加檔案上傳目錄中存在但是沒有發表在文章中的檔案. 這很可能因為會員新增了附加檔案但是最後沒提供文章. 您可以刪除這些附加檔案或者將其新增到存在的文章中. 新增附加檔案到文章中需要一個有效的文章ID, 您需要自己新增這個ID, 這個功能主要是幫助那些希望通過其他方式上傳檔案的人們, 可以把附加檔案(通常會很大)新增到已經發表的文章中.',
	'ADD_EXTENSION'						=> '新增副檔名',
	'ADD_EXTENSION_GROUP'				=> '新增副檔名組',
	'ADMIN_UPLOAD_ERROR'				=> '新增檔案中出錯: %s',
	'ALLOWED_FORUMS'					=> '允許的版面',
	'ALLOWED_FORUMS_EXPLAIN'			=> '允許在勾選的版面中發表指定的副檔名附加檔案',
	'ALLOWED_IN_PM_POST'				=> 'Allowed',
	'ALLOW_ATTACHMENTS'					=> '允許附加檔案',
	'ALLOW_ALL_FORUMS'					=> '所有版面中允許',
	'ALLOW_IN_PM'						=> '簡訊中允許',
	'ALLOW_PM_ATTACHMENTS'				=> '允許在私人簡訊中新增附加檔案',
	'ALLOW_SELECTED_FORUMS'				=> '只允許在下列勾選的版面',
	...
	'ATTACH_MAX_PM_FILESIZE_EXPLAIN'	=> '每個會員簡訊附加檔案的最大可用空間, 0表示無限制.',
	...
	'DEFINE_ALLOWED_IPS'			=> '設定允許的IP位址/主機名',
	'DEFINE_DISALLOWED_IPS'			=> '設定停用的IP位址/主機名',
	'DOWNLOAD_ADD_IPS_EXPLAIN'		=> '設定多個IP或主機名, 請使用多行輸入. 設定IP位址範圍, 請在起止間用破折號 (-), 萬用字元請用 * 號',
	'DOWNLOAD_REMOVE_IPS_EXPLAIN'	=> '您可以在瀏覽器中使用合適的鍵盤和鼠標操作組合刪除 (或 取消排除) 多個IP位址, 排除的IP位址背景為藍色.',
	...
	'EXCLUDE_ENTERED_IP'			=> '勾選這個以排除輸入的IP位址/主機名.',
	'EXCLUDE_FROM_ALLOWED_IP'		=> '從允許的IP位址/主機名中排除IP',
	'EXCLUDE_FROM_DISALLOWED_IP'	=> '從停用的IP位址/主機名中排除IP',
	...
	'MAX_ATTACHMENTS'				=> '每個文章的最大附加檔案數量',
	'MAX_ATTACHMENTS_PM'			=> '每個簡訊的最大附加檔案數量',
	'MAX_EXTGROUP_FILESIZE'			=> '最大檔案尺寸',
	'MAX_IMAGE_SIZE'				=> '最大圖片尺寸',
	'MAX_IMAGE_SIZE_EXPLAIN'		=> '圖片附加檔案的最大尺寸. 值都設為0px將停用這個功能.',
	...
	'REMOVE_ALLOWED_IPS'		=> '刪除或排除 <em>允許</em> 的IP位址/主機名',
	'REMOVE_DISALLOWED_IPS'		=> '刪除或排除 <em>禁止</em> 的IP位址/主機名',
	...
	'SECURE_DOWNLOADS_EXPLAIN'		=> '當開啟選項後, 下載將僅限於您設定的IP位址/主機名.',
