...
$lang = array_merge($lang, array(
	...
	'ADMIN_PASSWORD_CONFIRM' => '確定要管理員密碼',
	...
	'CONVERT_SETTINGS_VERIFIED' => '您輸入的資訊已經確定要，要開始轉換請點選下面的按鈕。',
	'CONV_ERR_FATAL' => '嚴重轉換錯誤',
	'CONV_ERROR_ATTACH_FTP_DIR' => '原討論區允許附加檔案使用FTP上傳，請停用FTP上傳選項並確定要指定了正確的上傳目錄，接著複製所有附加檔案到網站伺服器可以存取的目錄中，最後重新執行轉換程式。',
	...
	'DEFAULT_PREFIX_IS' => '轉換程式以指定的前綴找不到資料表，請確定要您輸入待轉換討論區的正確資訊，預設的資料表前綴於 %1$s 是 <strong>%2$s</strong>。',
	...
	'PRE_CONVERT_COMPLETE' => '全部的轉換準備步驟都已被成功完成，您現在可以開始進行實際的轉換操作。請注意，您也許需要手動調整部份項目，在轉換過後，特別要檢查設定的權限、必要時重建搜尋索引以及確定要檔案確實複製，像是人物與表情圖示。',
	...
	'BINARY_FILE' => '可執行檔',
	'BOT' => '爬蟲/機器人',
	'CHANGE_CLEAN_NAMES' => '確定要帳號沒有被多個會員使用的方法已經改變，新的方法發現有一些使用者使用了同樣的名稱，您必須刪除或是修改這些使用者的名稱來確定要一個使用者只對應一個使用者名稱，接著才能進行下一步。',
	'CHECK_FILES' => '檢查檔案',
	'CHECK_FILES_AGAIN' => '再次檢查檔案',
	'CHECK_FILES_EXPLAIN' => '在下一個步驟中，要更新的檔案都將被檢查 - 如果這是第一次檔案檢查，將花費一點時間。',
	'CHECK_FILES_UP_TO_DATE' => '資料庫的版本已經是最新，您也許想要執行一次檔案檢查來確定要所有的檔案都是 phpBB 的最新版本。',
	'CHECK_UPDATE_DATABASE' => '繼續升級過程',
	'COLLECTED_INFORMATION' => '收集到的檔案資訊',
	'COLLECTED_INFORMATION_EXPLAIN' => '下面列表顯示了需要更新的檔案資訊，請閱讀每個狀態前的資訊，確定要他們的意義以及您在升級程序中可能需要執行的步驟。',
	...
	'DATABASE_TYPE' => '資料庫類型',
	'DATABASE_UPDATE_INFO_OLD' => '在安裝目錄中的資料庫升級檔案是舊的，請確定要上傳正確版本的檔案。',
	...
	'NO_UPDATE_FILES_OUTDATED' => '沒有發現正確的升級目錄，請確定要上傳了相關的檔案。<br /><br />您安裝的版本似乎 <strong>不是</strong> 最新版本。版本 %1$s 的 phpBB 更新程式已經推出，請瀏覽 <a href="http://www.phpbb.com/downloads.php" rel="external">http://www.phpbb.com/downloads.php</a> 來取得正確的程式進行版本 %2$s 到版本 %3$s 升級程式。',
	'NO_UPDATE_FILES_UP_TO_DATE' => '您的版本已經是最新，沒有必要執行升級程式；如果您希望執行完整的檔案檢查，請確定要您上傳了正確的升級程式。',
	...
	'UPDATE_FILES' => '升級檔案',
	'UPDATE_FILES_NOTICE' => '請確定要您已經升級了討論區檔案，這個檔案只用來升級您的資料庫。',
	'UPDATE_INSTALLATION' => '升級 phpBB',
	'UPDATE_INSTALLATION_EXPLAIN' => '透過這個選項，您的 phpBB 將升級到最新版本。<br />過程中會檢查所有檔案的正確性，讓您可以在升級之前確定要所有檔案與差異。<br /><br />檔案升級可以透過兩個不同的方式。</p><h2>手動升級</h2><p>透過這個方式，您只需要下載本身異動檔案的集合，藉此避免遺失自己曾經修改過的部份。下載這個集合後您需要自己上傳檔案到正確的位置，完成後可以再次執行檔案檢查步驟來確定要檔案是否放在正確的位置。</p><h2>透過 FTP 自動升級</h2><p>這個方法與第一個差不多，但是不需要自行下載與上傳異動的檔案，程式會自動幫您完成。要使用這個方式，您需要知道自己網站的 FTP 登入資訊，完成後程式會引導您再次進行檔案檢查來確定要升級程序確實完成。<br /><br />',
	...
	'UPDATE_INSTRUCTIONS_INCOMPLETE' => '<h1>發現失敗的升級過程</h1> <p>phpBB 發現一個未完成的自動升級程序，請確定要您按照自動升級工具的引導進行；下面可以再次找到連結，或是直接到您的安裝目錄。</p>',
	'UPDATE_METHOD' => '升級方式',
