...
<INPUT type="button" value="Reset"
	onclick="javascript:location.href='lang_mngt.php';" class="btn">&nbsp;&nbsp;
<INPUT type="submit" value="go" class="btn"></DIV>
<INPUT type="hidden" name="file_name_escaped" value="" /> <INPUT
	type="hidden" name="file_name_escaped_cp" value="" /> <SCRIPT
...
function expl($tab, $lvl, $the_file, $array_name, $type, $info_lang, $version_id, $parent = null) {
    global $display;
    global $global_query;
    ...
    while (list($key, $val) = each($tab)) {
        ...
        if(gettype($val) == "array") {
            ...
            $info_lang = expl($val, $lvl, $the_file, $key, $type, $info_lang, $version_id, $array_name);
        } else {
            $i++;
            $info_lang["nbl"]++;
            $tmp_file = substr($the_file,1);
    ...
    if(($_POST['srctype'] == "sug_inst" && !isset($_POST['drop_lang'])) || ($_POST['srctype_cp'] == "sug_inst" && !isset($_POST['drop_lang_cp']))) {
        ...
        if(isset($_POST['action']) && $_POST['action'] == "compare_info") {
            ...
            if(isset($_POST['srctype']) && $_POST['srctype'] == "sug_inst") {
                ...
                foreach( $tab_files as $the_file ){
                    if(preg_match('/([^\/\\\\]*)\.lang\.php$/', $the_file, $match)) {
                        $lang = $match[1];
                    }
                }
                $path = $path_to_file;
        ...
        if(isset($_POST['action']) && $_POST['action'] == "import_dico") {
            ...
            print '<BR>';
            ?>
<table class="tabform-list" border="0" width="800" cellpadding="2"
	cellspacing="0" align="center" class='tabform-list'>
	<TR class=title>
		<TD colspan=3>Dictionary import results</TD>
	</TR>
	<TR>
