...
function showDicoEntry($query,$force_area ,$checkboxes = false,$show_next = false, $show_previous = false) {
    ...
    <TR><TD colspan=<?php echo $colspan?>>
    	<TABLE class=shell width=100%><TR>
    	<TD class=left witdh=50%><INPUT TYPE=submit name='back' value='Back' align=left class=btnform /></TD>
    	<TD class=right class=right witdh=50%><INPUT TYPE=submit name='next' value='Next' align=right class=btnform /></TD>
    	</TR></TABLE>
    </TD></TR>
    <TR>
        <?php echo $checkbox_hdr?>
        <TD width=6%>Id</TD>
        <TD width=47%>Reference</TD>
        <TD width=47% colspan="2">Translation</TD>
    </TR>
    <?php
    for ($i = 0; $i < count($results) ; $i++) {
        ...
        echo '<TR class="' . $class . '">';
        if ($checkboxes) {
            echo '<TD><INPUT TYPE="checkbox" NAME="del' . $results[$i]->id . '" class="checkbox" /></TD>';
        }
        echo '<TD width="6%">' . $results[$i]->id . '</TD>';
        echo '<TD width="45%"><div align="left" id="data' . $results[$i]->id . '">' . $results[$i]->ref . '</div></TD>';
        echo '<TD width="4%">';
        echo '<a href="#" onClick="document.getElementById(\'entry' . $results[$i]->id .
        '\').value = document.getElementById(\'data' . $results[$i]->id .
        '\').innerHTML; return false;">=</a>';
        echo '</TD>';
        echo '<TD width="45%">';
        if ($force_area) {
            $rows = intval(strlen($results[$i]->ref)/70) + 1;
        } else {
            $rows = intval(strlen($results[$i]->trx)/70) + 1;
        }
        $bool = ($rows>1 || ereg("'", $results[$i]->trx));
        
        if ($bool) {
            echo '<TEXTAREA tabindex="' . mktime() . '" cols="63" rows="' . $rows .
            '" name="entry' . $results[$i]->id . '" id="entry' . $results[$i]->id .
            '" >' . $results[$i]->trx . '</TEXTAREA></TD></TR>';
        } else {
            echo '<INPUT tabindex="' . mktime() . '" type="text" size="67" name="entry' .
            $results[$i]->id . '" id="entry' . $results[$i]->id . '" value="' . $results[$i]->trx .
            '" /></TD></TR>';
        }
        
    }
    if (count($results) >=20) {
        echo '<TR><TD colspan="' . $colspan . '"><TABLE class="shell" width="100%">
        <TR><TD class="left" witdh="50%"><INPUT TYPE="submit" name="back" value="Back"
        align="left" class="btnform" /></TD><TD class="right" class="right" witdh="50%">
        <INPUT TYPE="submit" name="next" value="Next" align="right" class="btnform" /></TD></TR>
        </TABLE></TD></TR>';
    }
    echo '</TABLE>';
