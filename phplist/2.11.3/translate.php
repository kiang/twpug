...
if($_GET['lang_file']){
  ...
  if(file_exists('admin/lan/'.$lang.'/'.$_GET['lang_file']))
    include 'admin/lan/'.$lang.'/'.$_GET['lang_file'];
  ...
  foreach($result1 as $key => $arr){
      if(array_key_exists($key, $en_lan))
      {
        ...
        if(preg_match('/'.chr(10).'/', $en_lan[$key]))
          echo '<textarea rows="5" cols="68" name="lan['.preg_replace(array('/\[/', '/\]/'), array('(((', ')))'), htmlentities($key)).']">'.stripslashes(htmlentities($arr,ENT_QUOTES,$encode)).'</textarea><br>';
        else
          echo '<input type="text" name="lan['.preg_replace(array('/\[/', '/\]/'), array('(((', ')))'), htmlentities($key)).']" value="'.stripslashes(htmlentities($arr,ENT_QUOTES,$encode)).'" size="80">';
